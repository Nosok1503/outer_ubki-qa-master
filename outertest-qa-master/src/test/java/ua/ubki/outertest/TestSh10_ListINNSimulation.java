...
//        LOGGER.info("Clean DBs before tests FINISHED");
    ...
    public void after() {
        ...
        if (varresponseTime1 < varresponseTime2 && varresponseTime1 < varresponseTime3) {
            writeDataLineByLine(csvPath + "/log/LogUptime.csv", new String[]{requestTime1.format(DateTimeFormatter.ISO_DATE_TIME), String.valueOf(varresponseTime1), kodRequest1, shablonRequest1, textRequestError1});
        } else {
            if (varresponseTime2 < varresponseTime1 && varresponseTime2 < varresponseTime3) {
                writeDataLineByLine(csvPath + "/log/LogUptime.csv", new String[]{requestTime2.format(DateTimeFormatter.ISO_DATE_TIME), String.valueOf(varresponseTime2), kodRequest2, shablonRequest2, textRequestError2});
            } else {
                writeDataLineByLine(csvPath + "/log/LogUptime.csv", new String[]{requestTime3.format(DateTimeFormatter.ISO_DATE_TIME), String.valueOf(varresponseTime3), kodRequest3, shablonRequest3, textRequestError3});
            }
...
//listINN\.get\(RandomUtils\.nextInt\(0\, listINN\.size\(\)\)\)\)
            ...
            .exec(session -> {
                LOGGER.info("varresponseTime1 < maxDelayTime && !requestError1: {} ", varresponseTime1 < maxDelayTime && !requestError1);
                //               commandExit=varresponseTime1 < maxDelayTime && !requestError1;
                return session.set("commandExit", varresponseTime1 < maxDelayTime && !requestError1);
            })
            .exitHereIf(session -> {
...
//Запит №2  Шаблон 10
            ...
            .exec(session -> {
                ...
                LOGGER.info("varresponseTime2 < maxDelayTime && !requestError2: {} ", varresponseTime2 < maxDelayTime && !requestError2);
                return session.set("commandExit", varresponseTime2 < maxDelayTime && !requestError2);
            })
            .exitHereIf(session -> {
                LOGGER.info("commandExit: {} ", session.getBoolean("commandExit"));
                return session.getBoolean("commandExit");
...
//Запит №4 Шаблон 11
            ...
            .exec(session -> {
                LOGGER.info("varresponseTime4 < maxDelayTime && !requestError4: {} ", varresponseTime4 < maxDelayTime && !requestError4);
                return session.set("commandExit", varresponseTime4 < maxDelayTime && !requestError4);
            })
            .exitHereIf(session -> {
                LOGGER.info("commandExit: {} ", session.getBoolean("commandExit"));
                return session.getBoolean("commandExit");
...
//Запит №5  Шаблон 11
            ...
            .exec(session -> {
                LOGGER.info("varresponseTime5 < maxDelayTime && !requestError5: {} ", varresponseTime5 < maxDelayTime && !requestError5);
                return session.set("commandExit", varresponseTime5 < maxDelayTime && !requestError5);
            })
            .exitHereIf(session -> {
                LOGGER.info("commandExit: {} ", session.getBoolean("commandExit"));
                return session.getBoolean("commandExit");
